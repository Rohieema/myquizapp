...
const newInter = setInterval(intervl, 1000);
...
nextButton.addEventListener("click", () => {
  ...
  if (currentIndex < quizData.length - 1) {
    ...
    currentIndex++;
    showQuestion();
    answerButtons.addEventListener("click", scoreCalc);
  } else {
    const next = document.getElementById("btn-there");
    console.log(next);
    next.innerHTML = `<button>Submit</button>`;
    // setInterval(intervl,1000)
  }
});
...
const scoreCalc = (e) => {
  ...
  answerButtons.removeEventListener("click", scoreCalc);
  if (e.target.id === "true") {
    score++;
